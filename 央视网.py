...
class Spider(Spider):
    ...
    兼容采用 Python Spider 协议的 TVBox/FongMi/OK影视/绿豆类客户端。
    """
    # ==================== 加密常量 ====================
    DELTA = 0x9e3779b9
    ROUNDS = 16
    ...
    # 将原始透明台标放入 480x680 竖版彩色卡面。背景按频道分组变化，
    # 避免所有卡片都是接近黑色；台标继续保持原比例，不会拉伸。
    PORTRAIT_LOGO_API = (
        'https://images.weserv.nl/?url={url}'
        '&w=480&h=680&fit=contain&bg={bg}&output=png&q=92&sharp=1'
    )

    # 深度适中的品牌色：保证白色台标清晰，同时比原来的黑蓝底更活泼。
    LOGO_PALETTES = {
        'cctv': ('d83b3b', 'e05a33', 'b93b69', '7157b7', '3568b8', '167c80'),
        'cgtn': ('155e75', '0f766e', '167c80', '256d85'),
        'pay':  ('7c3f91', '884c75', '6750a4', '4f5d95'),
        'sat':  ('2563a8', '2878a8', '32738c', '3d6494', '376aa6', '1f7a76'),
        'other': ('4d648d', '52677d', '3e6b67'),
    }
    ...
    LOGOS = {
        ...
        'gxpd': 'https://tuchuang.520xg.dpdns.org/file/1787125239198_sz.png',
    }
        ...
        for cid in cat['ids']:
            ...
            if ch:
                vod_list.append({
                    'vod_id': cid,
                    'vod_name': ch[3],
                    'vod_pic': self._portrait_logo(cid),
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        if play_url:
            # 返回央视频直链。网页端本身已有 /api/tvbox/play 播放代理，
            # 此处不能再套 Python getProxyUrl()，否则会形成双重代理，
            # 浏览器只能看到 00:00 并一直处于缓冲状态。
            return {
                'parse': 0,
                'url': play_url,
                'header': {
    ...
    def searchContent(self, key, quick=False, pg='1'):
        ...
        for cid, ch in self.CHANNELS.items():
            if keyword in cid.lower() or keyword in ch[3].lower():
                videos.append({
                    'vod_id': cid,
                    'vod_name': ch[3],
                    'vod_pic': self._portrait_logo(cid),
    ...
    def localProxy(self, param):
        try:
            ...
            is_m3u8 = (
                ...
                or upstream.lower().split('?', 1)[0].endswith(('.m3u8', '.m3u'))
                or response.content.lstrip().startswith(b'#EXTM3U')
            )
            if is_m3u8:
                text = response.content.decode('utf-8', errors='ignore')
                rewritten = self._rewrite_m3u8(text, response.url or upstream)
                return [
                    200,
            ...
            return [
                ...
                ('Proxy Error: %s' % error).encode('utf-8'),
            ]
    ...
    def _proxy_link(self, upstream):
        ...
        # 网页播放器通常依靠 URL 中的扩展名判断是否启用 HLS.js。
        # 代理地址本身没有扩展名，因此额外保留一个明文媒体后缀。
        path = upstream.split('?', 1)[0].lower()
        media_ext = '.m3u8'
        for candidate in ('.m3u8', '.m3u', '.m4s', '.mp4', '.ts', '.aac', '.key'):
            if path.endswith(candidate):
                media_ext = candidate
                break
        return '%s%surl=%s&media=%s' % (
            proxy_base,
            separator,
            quote(upstream, safe=''),
            media_ext,
    ...
    def _rewrite_m3u8(self, playlist, base_url):
        output = []
        ...
        for raw_line in playlist.splitlines():
            ...
            if line.startswith('#'):
                def replace_uri(match):
                    ...
                    return 'URI=%s%s%s' % (
                        match.group(1),
                        self._proxy_link(absolute),
                        match.group(1),
                    )
                output.append(uri_pattern.sub(replace_uri, raw_line))
            else:
                output.append(self._proxy_link(urljoin(base_url, line)))
        return '\n'.join(output) + '\n'
    ...
    def _portrait_logo(self, cid):
        original = self.LOGOS.get(cid, '')
        if not original:
            return ''
        group = self._logo_group(cid)
        palette = self.LOGO_PALETTES[group]
        # 使用稳定的字符和选择颜色；不同进程、不同设备显示保持一致。
        color_index = sum(ord(char) for char in cid) % len(palette)
        return self.PORTRAIT_LOGO_API.format(
            url=quote(original, safe=''),
            bg=palette[color_index],
        )

    def _logo_group(self, cid):
        if cid.startswith('cgtn'):
            return 'cgtn'
        if cid in self.CATS[2]['ids']:
            return 'pay'
        if cid in self.CATS[3]['ids']:
            return 'sat'
